...
game.state.add('play', {
  ...
  create: function() {
    ...
    this.eggLayfullWidth = this.eggLayBarFront.width;
    ...
    button.text = button.addChild(this.game.add.text(42, 5, 'Shape Default', { font: '12px Arial Black'}));
    button.details = {cost: 1};
    // Default Shape Bar
    this.defShapeBarBack = this.shapingInfoUI.addChild(this.game.add.sprite(window.innerWidth-600, 130, 'timer', 1));
    this.defShapeBarFront = this.shapingInfoUI.addChild(this.game.add.sprite(window.innerWidth-600, 130, 'timer', 0));
    this.defShapefullWidth = this.defShapeBarFront.width;
    ...
    //
    // 100ms 10x a second
    this.initBar(); // Init progress bars
    this.GameLoop = this.game.time.events.loop(this.system.interval, this.GameLoop, this);

  },
  initBar: function(barObj) {
    this.EggLayrect = new Phaser.Rectangle(0, 0, 0, this.eggLayBarFront.height);
    this.eggLayBarFront.crop(this.EggLayrect);
    this.DefShapeRect = new Phaser.Rectangle(0, 0, 0, this.defShapeBarFront.height);
    this.eggLayBarFront.crop(this.DefShapeRect);
  },
  updateText: function() {
    ...
    // Egg Lay Bar
    this.EggLayrect.width = Math.max(0, (this.genistars['egg'][2] / 100) * this.eggLayfullWidth);
    this.eggLayBarFront.crop(this.EggLayrect);
  },
  growEggs: function() {
    // Laying eggs
